...
export default function UserManagementClient({ initialUsers = [] }: UserManagementClientProps) {
    ...
    const users = storeUsers.length > 0 ? storeUsers : initialUsers;
    const isLoading = adminIsLoading.fetchUsers && storeUsers.length === 0;
    const isDeleting = adminIsLoading.deleteUser;
    ...
    useEffect(() => {
        if (hasInitialized) {
            fetchUsers({ q: searchTerm, sort: sortBy, role: filterRole });
        } else {
            setHasInitialized(true);
        }
    }, [searchTerm, sortBy, filterRole, hasInitialized, fetchUsers]);
