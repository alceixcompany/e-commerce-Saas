...
function MissingInfoModalInner({ onClose, user, onRefresh }: Omit<MissingInfoModalProps, 'isOpen'>) {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(getErrorMessage(err) || t('checkout.modal.saveError'));
        } finally {
            setIsSaving(false);
        }
    };
                ...
                <div className="p-8">
                    <div className="flex items-center gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-foreground/40 font-medium uppercase tracking-[0.2em] mt-0.5">
                                {t('checkout.modal.subtitle')}
                            </p>
                        </div>
                    </div>

                    <p className="text-sm text-foreground/60 leading-relaxed mb-6">
                        {!user?.phone ? t('checkout.modal.descComplete') : t('checkout.modal.descUpdate')}
                    </p>
...
export default function MissingInfoModal({ isOpen, onClose, user, onRefresh }: MissingInfoModalProps) {
    return (
        <AnimatePresence>
            {isOpen && (
                <MissingInfoModalInner
                    key={user?.phone || 'missing-phone'}
                    onClose={onClose}
                    user={user}
                    onRefresh={onRefresh}
                />
