...
function RegisterContent() {
	    ...
	    useEffect(() => {
	        const initPage = async () => {
	            try {
	                ...
	                if (isPreview) {
	                    tasks.push(dispatch(fetchPageBySlug('register')) as unknown as Promise<unknown>);
    ...
    if (!isInitialized) {
	    ...
	    const sections: RenderableSection[] =
	        isPreview && currentPage?.slug === 'register' ? ((currentPage.sections || []) as RenderableSection[]) : [];
