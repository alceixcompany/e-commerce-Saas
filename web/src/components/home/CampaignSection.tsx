...
const CampaignSection: React.FC<CampaignSectionProps> = ({ data, instanceId }) => {
    ...
    return (
        <motion.section
            ...
            onViewportEnter={() => setHasAnimatedOnce(true)}
            ...
            <div className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-12">
                {finalData.title && (
                    <motion.div
                        ...
                        animate={hasAnimatedOnce ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
                ...
                <div className={`grid gap-8 ${finalData.layout === 'split'
                    ...
                    {finalData.items?.map((item: Sections.CampaignItem, index: number) => (
                        <motion.div
                            ...
                            animate={hasAnimatedOnce ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0.98 }}
                            ...
                            <div className="absolute inset-0 p-8 md:p-12 flex flex-col justify-end items-center text-center">
                                {finalData.showBadge !== false && (
                                    <motion.span
                                        ...
                                        animate={hasAnimatedOnce ? { opacity: 1, y: 0 } : { opacity: 0, y: 10 }}
                                        transition={{ delay: 0.3 + (index * 0.1), duration: 0.5 }}
                                    >
                                        {t('common.exclusiveCampaign')}
                                    </motion.span>
                                )}
                                {finalData.showTitle !== false && (
                                    <motion.h3
                                ...
                                {finalData.showDescription !== false && (
                                    <motion.p
