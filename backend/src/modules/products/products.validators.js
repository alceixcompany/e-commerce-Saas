...
const createProductValidators = [
    ...
    body('variations', 'Variations must be an array').optional().isArray(),
    body('variations.*.label', 'Variation label is required').optional().notEmpty().trim(),
    body('variations.*.price', 'Variation price must be a non-negative number').optional().isFloat({ min: 0 }),
    body('variations.*.image', 'Variation image must be a string').optional().isString(),
